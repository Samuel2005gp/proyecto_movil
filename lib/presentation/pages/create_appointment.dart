...
class _CreateAppointmentScreenState extends State<CreateAppointmentScreen> {
      ...
      if (results[1].statusCode == 200) {
        ...
        _empleados = data
            .map((e) => {
                  'id': _toInt(e['id']),
                  'name':
                      '${e['firstName'] ?? e['nombre'] ?? ''} ${e['lastName'] ?? e['apellido'] ?? ''}'
                          .trim(),
                })
            .toList();
      }
  ...
  void _agregarServicio() {
    ...
    final servicio =
        _servicios.firstWhere((s) => s['id'] == _servicioSeleccionado);
    final empleado =
        _empleados.firstWhere((e) => e['id'] == _empleadoSeleccionado);
  ...
  Future<void> _save() async {
    if (_selectedHora == null) {
      SnackBarHelper.showError(context, 'Selecciona una hora de inicio');
      return;
    }
    if (_userRole == 'Admin' && _selectedClienteId == null) {
      SnackBarHelper.showError(context, 'Selecciona un cliente');
      return;
    }
    ...
    if (fechaCita.isBefore(fechaSolo)) {
      SnackBarHelper.showError(
          context, 'No puedes crear citas en fechas pasadas');
      return;
    }
    ...
    try {
      ...
      // El backend valida formato HH:mm exacto
      final horaStr = _selectedHora!;
      ...
      if (response.statusCode == 201 || response.statusCode == 200) {
        if (!mounted) return;
        SnackBarHelper.showSuccess(context, 'Cita creada exitosamente');
        ...
        try {
          ...
          if (msg is List) {
            ...
            errorMsg =
                msg?.toString() ?? error['error']?.toString() ?? errorMsg;
          }
        } catch (_) {}
        _showError(errorMsg);
      }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      isExpanded: true,
                      decoration:
                          const InputDecoration(hintText: 'Selecciona hora'),
                      items: _horas
                          .map(
                            ...
                            ..._serviciosAgregados.asMap().entries.map((entry) {
                              ...
                              return Container(
                                ...
                                child: Row(children: [
                                  ...
                                  GestureDetector(
                                    onTap: () => setState(
                                        () => _serviciosAgregados.removeAt(i)),
